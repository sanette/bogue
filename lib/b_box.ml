...
let default_background = Style.Solid Draw.(transp black);;
let default_border = Style.(border {
    color = Draw.(opaque grey);
    width = 1;
    style = Solid });; (* not used *)

let create ?width ?height ?(background = default_background) ?border () =
    ...
    | None ->
      ...
      begin match Var.get b.background with
        ...
        | Style.Gradient { Style.colors; angle } ->          
          gradientv3 canvas.renderer ~angle ~target colors;
