...
module Engine = struct
  ...
  let activate screen menu =
    if menu.active then ()
    else begin
        if not menu.always_shown then show screen menu;
        menu.active <- true
      end

  let close screen menu =
    print_endline "CLOSE";
    ...
    if not menu.always_shown && menu.active then
      begin
        ...
        Layout.fade_out ~duration menu.room
      end
  ...
  (* We could make it more efficient and stop going down a branch as soon as a
     ...
     not worth. *)
  let rec close_children screen menu =
  ...
  let close_others screen entry =
    ...
    List.iter (close_entry screen) other_entries
  ...
  let mouse_over screen entry =
    ...
    if entry.enabled && not entry.selected then begin
        highlight_entry entry;
        close_others screen entry;
        match entry.kind with
        | Menu menu ->
           if (not menu.active) && entry.parent_menu.active
           then activate screen menu
        | Action _ -> ()
      end
...
module Tmp = struct
  ...
  let rec compute_layouts entry =
    let layout =
      ...
      then let background = Layout.Solid Draw.(opaque grey) in
           Layout.empty ~background ~w:10 ~h:1 ()
      else format_label entry.label in
  ...
  (* Compute the room containing the menu, and its position wrt the parent
     entry. 'entry' should contain a submenu. *)
...
(* Return a copy of the tree with all Text labels replaced by Layouts *)
(* let rec compute_layouts entry =
 *   let layout = format_label entry.label in
 *   let content = match entry.content with
 *     | Action _
 *       | Separator -> entry.content
 *     | Flat list -> Flat (List.map compute_layouts list)
 *     | Tower list -> Tower (List.map compute_layouts list)
 *     | Custom list -> Custom (List.map compute_layouts list) in
 *   { label = Layout layout; content }
 *   
 * let get_layout entry =
 *   match entry.label with
 ...
 *               
 * let menu_formatter = function
 *   | Action _ -> failwith "menu_formatter acts on submenus, not actions"
 *   | Flat _ -> (fun list ->
 *     let background = Layout.Solid Draw.(opaque menu_bg_color) in
 *     let shadow = Style.shadow ~offset:(1,1) ~size:1 () in
 *     Layout.flat ~margins:0 ~background ~shadow list)
 *   | Tower _ -> (fun list ->
 *     (\* TODO add shadow. ceci ne marche pas *\)
 *     let shadow = Style.shadow ~offset:(1,1) ~size:1 () in
 *     let background = Layout.Solid Draw.(opaque menu_bg_color) in
 *     let l = Layout.tower ~margins:0 ~sep:0 ~background ~shadow list in
 *     Layout.expand_width l; l)
 *   | Custom _ -> (fun list -> Layout.superpose list)
 * 
 * (\* Return (x,y) option, the coordinates where the submenu should be placed when
 *    positioned in the same layout as the parent layout. *\)
 * let submenu_pos parent position = 
 *   let w, h = Layout.get_size parent in 
 *   map_option position
 *     (function | Below -> (0, h)
 ...
 * 
 * let next_submenu_position = function
 *   | Action _ -> failwith "next_submenu_position acts on submenus, not actions"
 *   | Flat _ -> print_endline "BELOW"; Some Below
 *   | Tower _ -> print_endline "RIGHTOF"; Some RightOf
 *   | Custom _ -> print_endline "NONE"; None
 *               
 * let get_entries = function
 *   | Action _ -> failwith "get_entries acts on submenus, not actions"
 *   | Flat list -> list
 *   | Tower list -> list
 *   | Custom list -> list
 * 
 * (\* Compute the room containing the menu, and its position wrt the parent
 *    entry. 'entry' should contain a submenu. *\)
 * let compute_room position entry =
 *   let layouts = List.map get_layout (get_entries entry.content) in
 *   let room = menu_formatter entry.content layouts in
 *   let parent = get_layout entry in
 *   room, submenu_pos parent position
 * 
 * (\* Convert an entry to an Engine.entry. Warning, this is not an obvious
 *    function, because Engine.entry is bidirectional, and hence cannot be created
 *    by a simple recursive loop. We need to use mutability: some fields are filled
 *    in later. *\)
 * (\* This should be called on a well prepared entry tree where all labels are
 *    layouts. *\)
 * let rec entry_to_engine position parent_menu entry =
 ...
 *     | Action a -> Engine.Action a, None
 *     | submenu ->
 *        let room, pos = compute_room position entry in 
 *        let menu = Engine.{
 *              pos;
 *              active = false;
 *              always_shown = false;
 ...
 *              room;
 *              parent_entry = None} in
 *        Engine.Menu menu, next_submenu_position submenu in
 *   let engine_entry = Engine.{kind;
 *                              enabled = true;
 *                              selected = false;
 *                              layout;
...
(* let create_old ~dst content =
 *   let t = create_menu content in
 *   Engine.init ~dst t;
