...
let uencode c =
  if c < 128 then Bytes.make 1 (Char.chr c)
    ...
    let c2 = 128 lor ((c lsr 6) land 31) in
...
(** split utf8 string into list of string letters *)
...
let bytes_first_letter s i =
  if i >= String.length s then 0
       ...
       else if c lsr 6 = 2 then failwith (Printf.sprintf "Wrong UTF8 at position %d. The string was probably cut at a wrong position." i)
